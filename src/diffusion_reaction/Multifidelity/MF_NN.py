...
def mfnn(repeat, ls_test, num_train, weight_decay, X_lo_train, y_lo_train):
    ...
    X_hi_test = np.array([[a, b] for a in np.linspace(0, 1, 101) for b in np.linspace(0, 1, 101)])
...
def main():
    ...
    num_train = 100
    weight_decay = 1e-6
    d = np.loadtxt(f"../../../data/diffusion_reaction/low_fidelity_{ls_test}.dat")
    output_list = []
    for repeat in range(100):
        y_lo_train = d[repeat][:, None]
        output = apply(mfnn, [repeat, ls_test, num_train, weight_decay, X_lo_train, y_lo_train])
