...
def mfgpr(repeat, ls_test, num_train, y_train_l):
    ...
    x_test_h = data_test['x_trunk_test']
    y_test_h = data_test['y_test'][repeat]
    x_train_l = data_test['x_trunk_test'][::12, :]
...
def main():
    ...
    for repeat in range(100):
        y_train_l = d[repeat].reshape(-1, 1)[::12, :]
        output = dde.utils.apply(mfgpr, [repeat, ls_test, num_train, y_train_l])
        output_list.append(output)
    output_list = np.concatenate(output_list, axis=0)
    np.save(f"predict_mfgpr.npy", output_list)
