...
def deeponet_together(repeat, ls_test, num_train):
    ...
    X_test = (np.tile(sensor_value, [5082, 1]), data_test["x_trunk_test"])
    ...
    return model.predict(X_test).reshape(1, 5082)


if __name__ == "__main__":
    ls_test = 0.2
    num_train = 50
    data_train = np.load(f"../../../data/poisson/poisson_train_ls_0.5_5082.npz")
    X_train_branch_origin = np.repeat(data_train["X_train0"], 5082, axis=0)
