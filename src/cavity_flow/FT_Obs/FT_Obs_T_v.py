...
def ft_obs_a(repeat, num_train):
    ...
    X_train_v_addition = np.tile(sensor_value, [num_train, 1])
    X_train_addition = data_test['x_trunk_test_select'][repeat]
    y_train_addition = data_test['v_test_select'][repeat]
    ...
    X_train = (np.concatenate([train_data["branch_train"][-5*10201:], X_train_v_addition], axis=0),
               np.concatenate([train_data["trunk_train"][-5*10201:], X_train_addition], axis=0))
    y_train = np.concatenate([train_data["v_train"][-5*10201:], y_train_addition], axis=0)
    ...
    model.train(iterations=iterations, display_every=100, model_restore_path=f"model_v/model")
...
if __name__ == "__main__":
    ...
    np.save(f"predict_ft_obs_t_v.npy", output_list)